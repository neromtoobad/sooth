...
- [x] **Phase 2** — DONE 2026-07-02. sUSD (Cep18X402 as one-token-for-everything), BinaryMarketFactory + MarketFactory on testnet; TWO markets created via ON-CHAIN factory (create_market works on casper backend); traders funded; liquidity seeded; smoke trade moved p_yes 50%→54.7% with explorer receipt. All hashes in deployments.json. Reads via CES events (lib/market-reader.ts, RPC-only)
- [x] **Phase 3** — DONE 2026-07-03 ~01:30 UTC. x402-gated /feed live (local reference facilitator, real on-chain settlements); momo+meanie traded 32 times on probe market; resolver resolved NO (dual-source price check) tx 8a367cf9…; claims paid on-chain; agent P&L real (momo 193.64, meanie 168.29 sUSD). Full lifecycle receipts in activity.jsonl + deployments.json
- [x] **Phase 4** — oracle ✓, consumer ✓, dashboard ✓ (Next.js in web/, dark, market cards + autoscaled p_yes chart + agent feed with explorer links + oracle curl panel; run `pnpm dev`). Trade-offs: CSPR.click human trading cut per plan fallback (read-only, agents are the story); Vercel deploy deferred to phase 5 (activity feed reads local activity.jsonl). REMAINING: vibes LLM trader (blocked: needs real Anthropic API key in .env — current one is a Venice key)
